...
Webアプリ開発に必要な機能のうち、フレームワークが提供しうるものは(ほぼ)全て用意されている。  
そのため、開発者は(フレームワークが提供し得ない)ビジネスロジック部分に集中できる。

Laravelを使うなら、Laravelの思想に沿わない使い方をするべきではない。
...
サービスクラスの肥大化を避けるため、責務ごとに小さなサービスに分割する（例：PostCreationService, PostUpdateServiceなど）。
...
DTO（Data Transfer Object）の使用は、APIでデータをやり取りする際の発想に非常に近い


使用する範囲

DTOはアプリケーション内部でのデータの移動ややり取りに使われることが多い（例：サービス層からコントローラー層へ）。
APIは、通常はアプリケーション間やシステム間でのデータのやり取りに使われます。
プロトコルや形式

APIは、HTTPやREST、GraphQLなどの通信プロトコルやフォーマット（JSONやXML）に基づいてデータをやり取りします。
DTOは特定のプロトコルに依存せず、アプリケーション内でのデータ構造の表現に使われます。
まとめ
