...
### 思想
初回アクセス時や外部サイトへの遷移など明確な理由がある場合を除き、すべてAjaxリクエストでSPAのようにページ遷移を行う。
...
※内部的には全てrouterを呼び出している。つまり、`router`が最も低レベルのAPIで、`Link`と`useForm`はその上に構築された便利なラッパー。
...
```js
{
  // データの配列。全データ中の細切れの一部（※->paginate(10)ならば10件分）
   "data": [
        {
            // Record...
        },
...
* コンポーネントの基本
  ...
  1. 処理: 内部に処理を持たせる
  1. イベント: 処理発火のきっかけ
  1. Props: 外部からオブジェクトとして値や「値を更新する関数」を渡せる
  1. コンポジション: レゴブロックのようにコンポーネントの組み合わせる

※keyは特殊なプロップなので、コンポーネントには渡されない。つまり、コンポーネント自体のルート要素にkeyを指定する必要はない。
